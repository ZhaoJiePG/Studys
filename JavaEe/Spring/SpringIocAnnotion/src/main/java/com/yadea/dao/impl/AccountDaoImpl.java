...
public class AccountDaoImpl implements IAccountDao {
    ...
    public void saveAccount(Account account) {
        try {
            runner.update("insert into account2(name,money)value(?,?)", account.getName(), account.getMoney());
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public void deleteAccount(Integer accountId) {
        try {
            runner.update("delete form account2 where id=?", accountId);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
